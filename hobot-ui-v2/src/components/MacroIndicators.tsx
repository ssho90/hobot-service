...
const ChartCard: React.FC<{ title: string; subtitle: string; data: any[]; color: string; icon: React.ReactNode }> = ({ title, subtitle, data, color, icon }) => (
  <div className="bg-white rounded-xl border border-zinc-200 p-4 shadow-sm hover:border-zinc-300 hover:shadow-md transition-all flex flex-col">
    <div className="flex items-start justify-between mb-4">
      <div>
        <h4 className="text-sm font-bold text-zinc-800">{title}</h4>
        <p className="text-[10px] text-zinc-400 uppercase tracking-tight">{subtitle}</p>
      </div>
      <div className={`p-1.5 rounded-lg bg-slate-50 border border-zinc-100 ${color.replace('stroke-', 'text-')}`}>
        {icon}
      </div>
    </div>

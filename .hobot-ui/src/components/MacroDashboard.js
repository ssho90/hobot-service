        ...
        <div className="news-list">
          ...
          {filteredNews.length === 0 ? (
            ...
            filteredNews.map(item => {
              // 한국어 모드일 때 DB에 있는 한국어 필드 사용, 없으면 원본(영어)만 표시
              const isKo = translationLang === 'ko';

              const displayTitle = isKo && item.title_ko ? item.title_ko : item.title;
              const displayDesc = isKo && item.description_ko ? item.description_ko : item.description;
              const displayCountry = isKo && item.country_ko ? item.country_ko : item.country;
              const displayCategory = isKo && item.category_ko ? item.category_ko : item.category;

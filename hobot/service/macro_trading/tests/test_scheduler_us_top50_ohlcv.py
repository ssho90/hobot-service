...
class TestUSTop50OHLCVScheduler(unittest.TestCase):
    def tearDown(self):
        schedule.clear()

    def test_run_us_top50_ohlcv_hotpath_calls_collector(self):
        fake_collector = Mock()
        ...
        with patch(
            ...
            result = scheduler.run_us_top50_ohlcv_hotpath(
                symbols=["AAPL"],
                max_symbol_count=5,
                lookback_days=7,
                continuity_days=120,
        ...
        self.assertEqual(kwargs["extra_symbols"], None)

    def test_run_us_top50_ohlcv_hotpath_skips_graph_sync_when_disabled(self):
        fake_collector = Mock()
        fake_collector.collect_top50_daily_ohlcv.return_value = {
            "target_symbol_count": 1,
            "fetched_rows": 10,
    ...
    def test_run_us_top50_ohlcv_hotpath_from_env(self):
        with patch.dict(
            os.environ,
            {
                "US_TOP50_OHLCV_SYMBOLS": "AAPL,MSFT",
                "US_TOP50_OHLCV_MAX_SYMBOL_COUNT": "12",
                "US_TOP50_OHLCV_MARKET": "US",
                "US_TOP50_OHLCV_LOOKBACK_DAYS": "45",
                "US_TOP50_OHLCV_CONTINUITY_DAYS": "180",
                "US_TOP50_OHLCV_START_DATE": "2026-01-01",
                "US_TOP50_OHLCV_END_DATE": "2026-02-19",
                "US_TOP50_OHLCV_INCLUDE_SUB_MP_UNIVERSE": "0",
                "US_TOP50_OHLCV_SUB_MP_MAX_SYMBOL_COUNT": "77",
                "US_TOP50_OHLCV_GRAPH_SYNC_ENABLED": "0",
                "US_TOP50_OHLCV_GRAPH_SYNC_INCLUDE_UNIVERSE": "0",
                "US_TOP50_OHLCV_GRAPH_SYNC_INCLUDE_EARNINGS": "0",
                "US_TOP50_OHLCV_GRAPH_SYNC_ENSURE_SCHEMA": "0",
            },
            clear=False,
        ), patch(
            "service.macro_trading.scheduler.run_us_top50_ohlcv_hotpath",
            return_value={"status": "ok"},
        ) as run_mock:
            result = scheduler.run_us_top50_ohlcv_hotpath_from_env()
        ...
        self.assertEqual(kwargs["sub_mp_max_symbol_count"], 77)
    ...
    def test_run_us_top50_ohlcv_hotpath_merges_sub_mp_symbols(self):
        fake_collector = Mock()
        fake_collector.collect_top50_daily_ohlcv.return_value = {
        ...
        with patch(
            ...
            result = scheduler.run_us_top50_ohlcv_hotpath(
                ...
                sub_mp_max_symbol_count=20,
            )

        kwargs = fake_collector.collect_top50_daily_ohlcv.call_args.kwargs
        self.assertEqual(kwargs["symbols"], ["AAPL"])
        self.assertEqual(kwargs["extra_symbols"], ["TLT", "SGOV"])
        self.assertTrue(result["sub_mp_universe_enabled"])
        self.assertEqual(result["sub_mp_extra_symbol_count"], 2)

    def test_resolve_country_sub_mp_symbols_normalizes_us_symbols(self):
        with patch(
            "service.macro_trading.scheduler._load_active_sub_mp_tickers",
            return_value=["AAPL", "005930", "0072R0", "tlt", "cash", " sgov "],
        ):
        ...
        self.assertEqual(result, ["AAPL", "TLT", "SGOV"])

    def test_setup_us_top50_ohlcv_scheduler_registers_job(self):
        schedule.clear()
        with patch.dict(
            os.environ,
            {
                "US_TOP50_OHLCV_ENABLED": "1",
                "US_TOP50_OHLCV_SCHEDULE_TIME": "",
            },
            clear=False,
        ):
            scheduler.setup_us_top50_ohlcv_scheduler()

        jobs = [job for job in schedule.get_jobs() if "us_top50_ohlcv_daily" in job.tags]

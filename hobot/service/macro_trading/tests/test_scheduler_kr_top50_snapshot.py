...
class TestKRTop50SnapshotScheduler(unittest.TestCase):
    def test_monthly_snapshot_job_skips_on_non_target_day(self):
        with patch(
            "service.macro_trading.scheduler.capture_kr_top50_snapshot"
        ) as capture_mock, patch(
            "service.macro_trading.scheduler.get_kr_corporate_collector"
        ) as collector_mock:
            result = scheduler.run_kr_top50_monthly_snapshot_job(
    ...
    def test_monthly_snapshot_job_captures_and_builds_diff_on_target_day(self):
        ...
        self.assertEqual(result["diff"]["added_count"], 2)
        self.assertEqual(result["diff"]["removed_count"], 1)
        self.assertEqual(result["diff"]["rank_changed_count"], 9)

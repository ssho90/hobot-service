...
- **사용처**: 계좌 성과 추적, AI 의사결정 시 현재 상태 참조
...
### 2.3 모듈 3: 내부 데이터 (계좌 손익)

#### 작업 항목
- [ ] **2.3.1** 계좌 상태 조회
  - [ ] 기존 `KISAPI.get_balance()` 활용
  - [ ] 자산군별 보유 현황 파싱
    - [ ] Stocks (S&P500, 나스닥, KOSPI)
    - [ ] Bonds (미국 장기채, 한국 단기채)
    - [ ] Alternatives (금, 달러)
    - [ ] Cash
...
#### 추가 작업 항목
- [ ] **2.3.4** ETF 티커 → 자산군 매핑 테이블 생성
  - [ ] 설정 파일의 ETF 매핑 정보를 DB에 저장
  - [ ] 자동 분류 로직 구현
...
#### 의사결정 필요 사항
- [ ] Few-shot 예시 데이터 (과거 성공 사례) 수집 방법

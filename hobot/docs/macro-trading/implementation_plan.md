...
- [ ] **1.3.2** 모니터링 시스템 구현
  ...
  - [ ] Slack 알림 통합 (기존 `slack_bot.py` 활용)
...
- [ ] **2.2.2** LLM 요약 분석
  ...
  - [ ] 프롬프트 설계
    - [ ] 연준 발표문(FOMC Statement)을 입력으로 받기
    - [ ] 핵심 내용 요약
    - [ ] 주요 정책 변화점 추출
  - [ ] JSON 스키마로 구조화된 출력 강제 (Pydantic)
  - [ ] 결과를 `economic_events` 테이블에 저장 (summary 컬럼 추가)
...
#### 의사결정 필요 사항
- [x] 경제 캘린더 데이터 소스: Investing.com → TradingEconomics로 변경 (Cloudflare 제한 회피)
- [x] 연준 발표문 수집 방법: API 우선, 없으면 크롤링 (결정 완료)
- [x] 센티멘트 분석: 제외, 요약만 수행 (결정 완료)

#### 추가 작업 항목
- [ ] **2.2.4** 연준 발표문 수집 빈도 정의
  - [ ] FOMC 회의 일정 기반 자동 수집
  - [ ] 수동 트리거 옵션 구현
...
fallback_rules = {
    "cpi_drop": {
        "condition": "CPI 전월 대비 하락",
        "action": {"Stocks": +10, "Bonds": -10}
    },
    "cpi_surge": {
        "condition": "CPI 전월 대비 0.5%p 이상 상승",
        "action": {"Bonds": +15, "Stocks": -10, "Alternatives": -5}
    },
    "yield_curve_inversion": {
        "condition": "DGS10 - DGS2 < 0 (금리 역전)",
        "action": {"Bonds": +20, "Stocks": -15, "Cash": -5}
    },
    "fed_rate_cut": {
        "condition": "FEDFUNDS 전월 대비 하락",
        "action": {"Stocks": +15, "Bonds": -10, "Alternatives": -5}
    },
    "unemployment_surge": {
        "condition": "실업률 전월 대비 0.3%p 이상 상승",
        "action": {"Bonds": +10, "Stocks": -5, "Cash": -5}
    }
}
```
